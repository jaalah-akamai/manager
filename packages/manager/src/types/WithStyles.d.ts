...
export type PropsFunc<Props extends Record<string, unknown>, T> = (
  props: Props
) => T;
...
export interface CreateCSSProperties<Props extends Record<string, unknown> = {}>
 ...
 * - the `values` are Record<string, unknown>s that represent CSS rules (`React.CSSProperties`).
 *
 ...
 * @internal
 */
export type ClassKeyInferable<Theme, Props extends Record<string, unknown>> =
  | string
  | Styles<Theme, Props>;
